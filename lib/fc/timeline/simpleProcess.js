...
exports.process = function (args, opts) {
    ...
    // 是否是文件，如果是则简单处理
    var stat = fs.statSync(path.resolve('.', fileName));
    if (stat.isFile()) {
    ...
    if (fileName) {
        var fstat = fs.statSync(path.resolve('.', fileName));
        if (fstat.isDirectory()) {
            var list = fs.readdirSync(fileName);
            var result = [];
            list.forEach(function (item, index) {
                if (!ignore[item]) {
                    var itemStat = fs.statSync(path.resolve(fileName, item));
                    if (itemStat.isFile()) {
                        var processResult = processSingleFile(path.resolve(fileName, item));
