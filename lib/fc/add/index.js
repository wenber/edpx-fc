...
exports.process = function (args, opts) {
    ...
    var typeMapper = {
        ...
        mainHtml: 'main.html',
        util: 'util.js',
        GET_basicInfo: 'GET_basicInfo.js',
        component: 1
    };
    ...
    // 否则应该是 efc add sth.js 或者 efc add sth
        ...
        if (args[0] in especialFile) {
            targetPath = './' +  typeMapper[args[0]];
        }
        else {
            targetPath = './' + args[0];
        }
    ...
    var isModule = false;
    if (!(fileName in especialFile)) {
        isModule = !(/\w+\.\w+$/.test(targetPath)) || /\/$/.test(targetPath);
    }
