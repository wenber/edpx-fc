...
exports.process = function (args, opts) {
    // 处理二级命令先
    var secondaryCommand = args[0] || 'simple';
    switch (secondaryCommand) {
        case 'jsLine':
            // 读取pattern
            var pattern = opts.pattern;
            if (!pattern) {
                edp.log.error('应当通过 --pattern= 来指定一个名称的匹配，字符串');
                return;
            }
            // 读取file
            var file = opts.file;
            if (!file) {
                edp.log.error('应当通过 --file= 来指定数据文件的位置');
                return;
            }
            require('./jsLine').process(pattern, file)
            break;
        case 'simple':
        default:
            var result = require('./simpleProcess').process(args, opts);
