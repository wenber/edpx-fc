...
exports.process = function (args, opts) {
    ...
    spawn('edp', ['project', 'init', currPath]).on('close', function (code) {
        ...
        // 引入fc库及相关依赖
        spawn('edp', ['import', 'fc-core','fc-view', 'fc-ajax', 'fc-storage', 'ef']).on('close', function (code) {
            if (code !== 0) {
            ...
            spawn('efc', ['add', '/entry/404']).on('close', function (code) {
                if (code !== 0) {
                    console.log('add 404 process exited with code ' + code);
                }
            });

            // 添加500模块
            spawn('efc', ['add', '/entry/500']).on('close', function (code) {
                if (code !== 0) {
                    console.log('add 500 process exited with code ' + code);
                }
                edp.log.info('初始化完成！');
            });
        });
